...
- **斜杠菜单排序 + /resume 命令 + tui 设置命名空间注册（0.2.1 后续，本轮）**：① **斜杠菜单排序**——原菜单按目录插入序展示（运行时命令 → TUI 原生 → 技能），改为 `matchCommands` 统一排序：空查询（刚输入 `/`）按显示名字母序全量展示，查询命中按前缀加权 + 子序列打分、同分按字母序打破平局；纯展示层排序，提交分辨率（label 前缀 find）原序不变，`/q` 等歧义前缀的解析行为不受影响；② **新增 `/resume` 斜杠命令（别名 `/r`）**——启动 `--browse` 会话选择器与 cc/pi `Ctrl+R`（opencode `<leader>l`）的斜杠入口：列出历史会话（子会话缩进、persisted/live 徽标、相对时间），选中即 swap 切换；③ **会话 picker 标题缓存 + 空闲回填**——平台侧 `readTitle` 对每个已持久化会话整读日志（大日志一次 0.5s+ 主线程解压解析，用户库 170+ 会话实测无上限并发直接卡死面板：标题不显示 + 上下键卡顿）；改为：标题先查 TUI 自有缓存（`~/.dsh/tui-titles.json`，会话切换/退出/重命名时写入），命中直接显示名称，未命中短 id 占位；后台仅在用户停止按键的空闲窗口（600ms）逐个串行 readTitle，解析一个就地刷新一行并落缓存，用户按键立即暂停（picker 每个键击经 `onSessionPickerActivity` 上报），选中/取消终止回填；**启动期初始化回填（跨版本恢复）**——升级后首次运行缓存为空，run() 在 app.start 后 spawn `backfillTitleCache`：全局空闲（任何键击经 `onUserActivity` 上报暂停、`doc.busy` 跳过）时串行补名，每次启动限量（`titleBackfillCap`，默认 50）约束主线程负担，读满 toast 汇报（`titlesBackfilled` 双语），quit 即终止；④ **tui 设置命名空间注册**——`apply()` 用 `installSettingsSection` 注册 `tui` 命名空间（schema：`enterBehavior: queue|steer`、`anim: on|off`），修复 `/settings` 的 Enter 行为/动画写入因命名空间未注册而被 settings 服务拒绝、`.catch(() => {})` 静默吞掉导致重启丢失的 bug（settings.yaml 从未出现 `tui:` 段）；E2E surface 场景补 `/resume` 步骤；单测 316→327。
...
- **regular 模式备忘补充（2026-08-20，三方实证）**：主流 AI 编码终端均提供「主屏 scrollback 渲染 ⇄ alternate screen 全屏」两条路径并允许切换，本备忘的 regular 路径即对齐其中主屏形态：
  - **Claude Code**：classic renderer（默认，主屏渲染保留 scrollback）⇄ fullscreen renderer（research preview，`/tui fullscreen` 切换、`/tui default` 切回，画在 alternate screen 缓冲"像 vim/htop"）。证据：官方文档 [Fullscreen rendering](https://code.claude.com/docs/en/fullscreen)；实现级佐证 anthropics/claude-code issue [#79229](https://github.com/anthropics/claude-code/issues/79229)（"TUI redraw without alternate screen"——v2.1.215 默认流式写主屏 + `CUP 1;1` 全屏重绘、从不 `DECSET 1049h`）与 [#42670](https://github.com/anthropics/claude-code/issues/42670)（v2.1.89+ 切 alt buffer 后 scrollback 丢失被老用户视为回归）。
  - **opencode**：交互 TUI（`opencode`，bubbletea/opentui，[官方文档](https://mintlify.wiki/opencode-ai/opencode/usage/interactive-mode)确认 "launches in an alternate screen buffer"）⇄ run 命令的 **direct-mode scrollback**（`packages/opencode/src/cli/cmd/run/scrollback.surface.ts` 注释原文 "Retained streaming append logic for direct-mode scrollback"；split-footer 架构：转录是 append-only 的 scrollback 区域，底部 footer 是可变的输入/状态控制面）。
  - **pi（本引擎）**：`TuiMainScreen`（regular，保留 scrollback）⇄ `TuiAltScreen`（viewport，当前所用）——pi-tui README 明示 "Interchangeable Renderers: main-screen and alternate-screen implementations"。
  - **对实现的修正**：overlay 机制（`overlayStack`/`compositeTuiLine`/`resolveOverlayLayout` 锚定/`nonCapturing`）在 `TuiBase` 层、**两种模式都可用**（pi 0.84.1 `tui.js` 的 overlay 合成在主屏渲染同样执行）——上备忘"overlay 锚定退化为堆叠渲染"基于更早判断，需修正为"锚定相对当前视口可用，但内容滚进 scrollback 后定位漂移"；上翻 scrollback 后差分渲染会把视口拉回底部是 regular 的固有硬伤（vim/less 因此切 alt-screen）。
  - **轻量参考形态**：若未来实现，可参考 opencode run 的 split-footer（append-only 转录 + 底部控制行），比完整重写布局轻；「右键/滚轮归终端原生」在该形态天然成立（应用不请求 SGR 上报），正是 2026-08-20「右键默认归 Warp」决策的正解（当前 alt-screen 下实测 Warp 仍不接管右键，已搁置）。
- **regular 模式实现记录（2026-08-20，骨架已落地）**：`--regular` / `DSH_TUI_REGULAR=1` 启动 regular 渲染（TuiMainScreen）。实现：`piTuiInternals.createTui(terminal, mouse, regular)` 分支到 `TuiMainScreen`；`PiTuiApp` 双布局——alt-screen 走 VStack+ScrollView，regular 走文档流（header/面板/转录/状态槽/footer/编辑器 `addChild` 顺序堆叠，TuiBase.render 逐 child 拼行）+ BottomPad 手动钉底（`updateBottomPadding` 以终端高度为视口）；能力降级：ScrollView 不构造（`isFollowingEnd` 恒 true → 回底提示/B16 计数自然不出现）、Ctrl+F 搜索跳转提示不可用（终端原生 Cmd+F 替代）、hookAltScreen 仅 `isViewportTUI` 时挂、鼠标归终端原生。`cordis.patch.yml` 的 tui-runner config 补 `regular: !!js ctx.tuiStartup.regular`（缺这行时 flag 静默丢失，E2E 以 `\x1b[?1049h` 断言揪出）。验证：单测（regular 渲染/输入/搜索降级 + `--regular` 解析）+ E2E 第 9 场景（flag 版 + 主屏断言）。**默认模式翻转（2026-08-20，用户决策）**：regular 成为默认渲染模式——`PiTuiApp` 解析 `options.regular ?? (options.fullscreen === true \|\| DSH_TUI_FULLSCREEN=1 ? false : DSH_TUI_REGULAR=1 \|\| true)`；`--fullscreen`/`DSH_TUI_FULLSCREEN=1` 显式切回 alt-screen 视口（旧默认），`--regular`/`DSH_TUI_REGULAR=1` 保留显式语义。E2E 既有 8 场景加 `--fullscreen` 保持原覆盖，regular 场景改用默认启动（`\x1b[?1049h` 主屏断言锁定「默认不切 alt screen」）。测试 mount 默认注入 `regular: false` 维持既有 alt 语义。
  **完成（同日后续轮）**：增量追加（`CachedTranscript` 行缓存容器——条目视图渲染行按宽缓存、applyState 更新时失效，流式只重渲染变化条目，长会话从 O(全会话×重渲染) 降到 O(缓存拼接)）、钉底精确高度（终端高度 - chrome 动态高度 - 转录内容，BottomPad 只在变化时 setHeight + 失效）、overlay 验证（hotkeys/slash 菜单在 regular 视口正常渲染，TuiBase 层共用）。- **regular 模式备忘补充②（2026-08-20，Claude Code classic 能力边界实证）**：本仓库 regular 路径下**无法实现**的能力，Claude Code 在其 classic（主屏/regular）renderer 里**同样没有**——这些能力官方全部放在 fullscreen（alternate screen）renderer 里。逐项对照（官方 [interactive-mode.md](https://code.claude.com/docs/en/interactive-mode.md) 与 [fullscreen.md](https://code.claude.com/docs/en/fullscreen.md)）：应用内对话搜索/跳转（classic 无，靠终端原生 Cmd+F 搜 scrollback；fullscreen 内置搜索 + `[` 把对话写进原生 scrollback 供 Cmd+F/tmux copy-mode 搜索）、鼠标响应（`/` 与 `@` 列表 hover/点击标注为 fullscreen 专属）、滚动管理（Auto-follow/Mouse wheel scrolling/Scroll the conversation 均为 fullscreen 章节；classic 整屏 `CUP 1;1` 重绘会拉回视口并产生 scrollback 重复行——[issue #79229](https://github.com/anthropics/claude-code/issues/79229)）、免闪烁（fullscreen 原话 "eliminates flicker"，classic 会闪烁）、长会话内存平（fullscreen 原话 "keeps memory usage flat"，classic 随会话增长）。三个细节：① `Ctrl+E`（展开全部内容）是 **classic 独有**（"Available in the classic renderer only"——classic 因重绘成本截断内容，需此补丁键；fullscreen 虚拟化渲染反而不需要）；② fullscreen 想要 scrollback 得手动按 `[` 导出——alt screen 用户反向向 regular 借能力，两形态互补而非替代；③ `Ctrl+R` 历史搜索在 classic 为内联 reverse search、fullscreen 为可切范围的搜索对话框，同一功能两套实现。**决策意义**：官方把 classic 定为"2026-05-06 前老用户保留"、新用户默认 fullscreen——官方演进方向即对"regular 作主路径"的否定；本仓库 regular 只适合可选形态（如 `--regular`），且可白嫖 Claude Code 的 `[` 设计：alt-screen 下加"导出转录到 scrollback"快捷键即可补上"终端 Cmd+F 搜索对话"的 regular 体验（低成本）。
...
- **规划/边界/自描述批（2026-08-22，v0.3.0 后）**：① 文档栈——`docs/BOUNDARY-DESIGN.md`（四判据 P1–P4 + 边界红线「dsh 共享状态零 sidecar 冒名」+ 已完成功能回审 §6）、`docs/PLAN-ROADMAP.md`（未动工 44 项 6 阶段：项/优先级/边界判定/前置/验收）、`docs/PLAN-BUNDLED-SKILL.md`；② **tui 操作手册技能随插件注册**（自描述能力=唯一例外，BOUNDARY §6.2）——`cordis.patch.yml` restate `skill-filesystem` 行 `customSkillDirs: [!!js "…createRequire(new URL('package.json', ctx.baseUrl)).resolve('@mcswift/dsh-tui/package.json')…+ '/skills'"]`（官方 `agent.cordis.yml` 同款：安装单元自带 skills/ 注册进宿主 registry；`!!js` 求值 = `with(ctx){eval}`，同 id restate 顶层整体替换安全）；`skills/tui/SKILL.md` 随包（`package.json files` + `skills`）；E2E 新场景 `--only-skills`（/ 菜单 whenToUse 断言）。
- **观察类与 cc 视觉批（2026-08-22）**：① C1 TPS 仪表——fold 为流式 assistant 记录 `decodeSamples`（text-delta 24 样本窗，thinking 不计），`stats.ts` 纯函数 `liveGauge`（近 8 样本窗口 chars/4≈tok、8 格 `▰▱`、≥50 success/≥20 warning/<20 error）+ `sparkline`（12 样本 min-max 归一 `▁▂▃▄▅▆▇█`）；busy 行实时 gauge + 回合后条目 footer 火花线；② A3 `/context`——`contextReport` 纯函数按注入 kind 分组（`【skill-catalog】` 节 + 首行 60 字预览）经 notice+detail 展开；③ A18 `/tips`——`TipsPanel` 覆盖面板（5 组双语：快捷键/命令/工作流/个性化/避坑）；④ V3 thinking 时钟——折叠行 `Thinking for Ns`（`setThinkingClock(firstChunkAt, committedAt)`：流式 Date.now 实时、committed 定格）；⑤ V4 输入框权限语义边框（cc 预设 `editor.borderColor = permissionTone(preset)` 运行时赋值，pi 还原主题边框；`❯` 前缀与 B15 同 seam 受限）；⑥ V5 busy 行 `↓ N tokens`（cc 预设，decodeSamples 字符累计/4 近似——流式无逐 token usage）。
- **会话/交互批（2026-08-22）**：① D2 会话 MRU——`tui-mru.json` sidecar（epoch、LRU 300），picker 先按 MRU 排 records 再 map（fillTitlesLazily 索引对齐）；② D1 /resume 行元数据——相对时间 + agentPreset + 子会话计数（仅 header 字段；模型/大小/Tab 预览记录为后续增强）；③ A14 `/workspace resume|rename|open [目标]`——open 支持 `~`/绝对/相对/`file://` URI（`resolveWorkspaceTarget`）并新建会话；rename 复用 H11 单段名校验 + fs.rename（抽 `renameWorkspaceDir`）；④ B5 Alt+R 输入历史搜索（Ctrl+R 已被会话/重命名占用）——`showHistorySearch` 经 showChoicePicker 回填输入框；⑤ B6 搜索面扩展（assistant 含 thinking、notice 含 detail）+ 结果循环跳转 Ctrl+N/Alt+N（`jumpToHit` + 位置 toast；`n/N` 与打字冲突）；⑥ B7 Shift+Up 消息选择模式（＝焦点环入口，`selectionMode` 下 ↑/↓ 逐条移动、Enter 交条目组件、Esc 复位）；⑦ B12 `!!cmd` 静默执行报名 `⚙ local <cmd> · N 行输出` + detail 全文（ExpandableNoticeView `│` 缩进 dim 展开；聚焦提示行分型 localHint/expandHint）；⑧ F3+V8 footer 三档 full/compact/minimal（`/settings` 新行 + tui 命名空间 `footerMode` 持久化 + `FooterLine.set(…, mode)` 即时重绘）。
- **深度能力批（2026-08-22）**：① A13 /trace——`matchTraceQuery` 纯函数查询语言（`tool:`/`kind:`/`turn:`/`err:` 前缀 + 关键词 AND；`>10s`/`tok>1k` 因行为行无结构化字段记录不可行），面板 `[`/`]` 跳错误行、`{`/`}` 跳轮次边界（`jumpTo` 环回）；② A16 `/thinking`——`setHideThinking`（与 Ctrl+T 共用 `applyThinking`）；③ A12 `/btw [问题]`——侧问浮层（`ctx.get('llm').stream` 直调当前模型、无工具单轮、AbortController 中止上一个；`BtwPanel` 瞬态 overlay：`c` 复制/Esc 关；**不写日志不进文档流**——fold 零改动）；④ 探针结论：A9 ✅ 已覆盖（`/config` addProviderWizard=克制版）、D3 ✅ 记录不做（无删除 API）、A10 记录降级（`agentPresets` 在 agent 组合 + `/preset` 命令名被键位/主题预设占用）。
- **观察渲染批（2026-08-22）**：① C3+A15 帧动画——`src/app/pi/frames.ts` 自建帧表（star/moon/dots，勿 import dsh-working-activity——不在依赖树）+ `StatusSlot.setFrames`（busy 中重建重启）+ `/activity`（选择器/`frames <id>` 直切/tui 命名空间持久化）+ busy 行 `⚠ ctx N%` 压力前缀（`contextPercent` 纯函数，≥80 amber/≥95 red）；② F1 自定义 JSON 主题——`$DSH_HOME/tui-themes/<名>.json`（colors 覆盖：已知语义角色（resolveHex 探测）+ 合法 hex 才收，损坏/未知键/路径穿越跳过）+ palette 覆盖层（`applyCustomThemeColors`/`customColor`，resolveHex 优先）+ `/theme <名>` 热切换、选择器（自定义在前）、持久化、启动回填；base（明暗）简化记录：沿用当前预设变体。
- **决策落地批（2026-08-22，用户拍板）**：① A21 ⛔ 关闭（先不做——偏离任务目标；BOUNDARY §3.2 判定保留）；② B4 📐 专项设计 `docs/DESIGN-ATTACHMENTS.md`（数据流/交互/渲染三级降级 Kitty→iTerm2→占位/存储去重/三期 P1–P3/测试/风险；实现押后）；③ E1+F4 分屏 diff——`SplitDiffText`（numbered diff 块配对左旧右新 + `│` 分隔；auto=≥110 列/split=强制/unified=单栏）+ `/settings`「Diff 布局」行（tui 命名空间 `diffLayout` 持久化 + 启动回填 + refreshTheme 热重建）；④ D4 子会话折叠——picker 默认只列父会话 + 顶行 toggle（`展开 N 个子会话/收起`，重开刷新行集），展开后子行 `↳` 缩进，fillTitlesLazily 仅回填可见行；⑤ F2 16 色 ANSI 回退——`ansiPaletteSet`（web 色板 vars 逐色最近 ANSI 近邻映射，RGB 亮度加权 0.30/0.59/0.11）+ `/theme` 第五预设 `ansi`；⑥ B11 OSC 0 tab 标题——忙时 `⠂/⠐` 两帧 2s 旋转、空闲 `✦`（`piTuiInternals.tabTitleWriter` 注入、NODE_ENV=test 跳过、stop 清理并清空；焦点检测简化记录——xterm 焦点协议 seam 未实现）。

- **split-footer 架构批（2026-08-26，D1）**：修复 regular（TuiMainScreen 主屏）下「内容超屏后输入框随内容流滚出视口 + 滚动携带输入框」——根因 = pi 主屏为整树线性输出、无「视口底部固定行」概念，chrome 行位于 newLines 末尾（行号 > 屏高）后被差分/钳位处理推离视口（任何终端如此，非 Warp 特有；CC classic 的解法 = 每帧 `CUP 1;1` 全屏重绘，代价 scrollback 重复行，见 anthropics/claude-code #79229/#49086/#46834）。**实现（对齐 CC classic 语义、规避其缺陷）**：新 `TuiMainScreenPinned extends TuiMainScreen`——`doRender()` 后把固定 chrome（状态槽/footer/编辑器）`CUP` 定位到视口底部重绘（`startRow = rows - chromeLines + 1`）；chrome 由 `PiTuiApp.paintChromeLines` 组装并从**渲染树拆出**（regular 分支不再 addChild statusSlot/footerLine/editor——输入经 pi 的 `focusedComponent` 直接分发，与树内/树外无关，已核实）：① 内容流（header/面板/转录）照常进 scrollback，滚动只携带内容；② chrome 行画在视口内 → **零 scrollback 重复行**（非 CC classic 全屏重绘）；③ 编辑器 `CURSOR_MARKER` 剥离 + 光标行列经 marker 位置定位（`[<row>;<col>H`），硬件光标/IME 锚定保持；④ BottomPad 钉底改为「视口 - 内容树 chrome - 固定 chrome - 转录内容」。验证：421 单测（新增超屏 CUP/贴底/marker 断言）+ 完整 PTY E2E 全绿；已知：chrome 绘制异常仅首次留痕（不影响内容渲染）；若 pi 上游未来提供 fixed-bottom rows API，本类可退化为上游适配层（提交上游 feature 建议是后续动作，见 PLAN-ROADMAP 附注）。
