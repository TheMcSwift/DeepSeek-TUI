...
import type { WorkingIndicatorOptions } from "./extensions-types.ts";
import { theme } from "../theme/theme.ts";
import { CountdownTimer } from "./countdown-timer.ts";

// The TUI interrupts with Esc (Claude Code style); pi 0.84.1 has no
// "app.interrupt" definition, so the vendored hints spell the key directly.
...
export class RetryStatusIndicator extends StatusIndicator {
	...
	constructor(ui: TUI, attempt: number, maxAttempts: number, delayMs: number) {
		const retryMessage = (seconds: number) =>
			seconds > 0
				? `Retrying (${attempt}/${maxAttempts}) in ${seconds}s... (esc to cancel)`
				: `Retrying (${attempt}/${maxAttempts})... (esc to cancel)`;
		super(
			"retry",
			ui,
