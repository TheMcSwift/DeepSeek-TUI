...
| 能力 | 判定 | 独立形态 | 理由 |
|---|---|---|---|
| **A21 打包技能（7 个）** | ✅ **最典型越权**（⛔ 已关闭：2026-08-22 用户决策「先不做——偏离任务目标」，判定保留、激活需重新决策） | **独立技能包**（dsh 技能生态：`~/.dsh/skills/<名>/SKILL.md`，可独立仓库）+ 安装脚本（同名覆盖语义与远程一致） | 技能是 **agent 能力**（SKILL.md 指令），headless / web 同样消费（P1 命中）；与 TUI 代码零耦合；放 TUI 包内 = 客户端私有化了一份本属于生态的能力 |
...
| Backlog 项 | 边界结论 |
|---|---|
| A21 打包技能 | **独立技能包**（唯一显著越权项；⛔ 已按用户决策关闭，2026-08-22，判定保留） |
