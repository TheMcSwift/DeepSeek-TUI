...
- **cordis 服务访问**：只允许 `ctx.get(...)` 结构读取 host 组合（`commands`/`skills`/`sessionQuery`/`sessionTitle`/`jobs`/`userQuestions`/`settings`）+ cordis 事件挂接（如 `user-questions/request` waterfall）；不 import 未在 profile 依赖树中的 dsh 包（会破坏 out-of-tree 约束；仅 `import type {}` 聚合事件类型允许，如 `dsh-tool-todo`）。
- **settings 命名空间**：TUI 持久化设置占用 `tui` 命名空间，`apply()` 内用 `ctx.settings.register('tui', schema, options)` 注册（schema 与 base 默认值在 `src/index.ts`）；`update('tui', …)` 依赖该注册，删除会让 `/settings` 的 Enter 行为/动画写入静默失效。
